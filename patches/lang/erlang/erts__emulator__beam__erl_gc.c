...
--- erts/emulator/beam/erl_gc.c.orig	2011-10-03 18:12:07.000000000 +0000
...
@@ -1064,6 +1078,13 @@ do_minor(Process *p, Uint new_sz, Eterm*
...
@@ -1285,6 +1306,13 @@ major_collection(Process* p, int need, E
     sys_memcpy(n_heap + new_sz - n, p->stop, n * sizeof(Eterm));
     p->stop = n_heap + new_sz - n;
...
@@ -1955,6 +1983,14 @@ grow_new_heap(Process *p, Uint new_sz, E
...
@@ -1993,6 +2029,14 @@ shrink_new_heap(Process *p, Uint new_sz,
         HEAP_TOP(p) = new_heap + heap_size;
         HEAP_START(p) = new_heap;
     }
+
